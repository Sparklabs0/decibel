...
export default function UserUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = userRecord
      ? { ...initialValues, ...userRecord, Notes: linkedNotes }
      : initialValues;
  ...
  React.useEffect(resetStateValues, [userRecord, linkedNotes]);
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          notesToUnLink.forEach((original) => {
            if (!canUnlinkNotes) {
              throw Error(
                `Notes ${original.id} cannot be unlinked from User because userID is a required field.`
              );
            }
            promises.push(
              DataStore.save(
